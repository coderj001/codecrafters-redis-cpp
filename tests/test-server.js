  ...
  async cleanup() {
    ...
    if (this.serverProcess) {
      try {
        ...
        try { if (this.serverProcess.stdout) this.serverProcess.stdout.removeAllListeners(); } catch (e) {}
        try { if (this.serverProcess.stderr) this.serverProcess.stderr.removeAllListeners(); } catch (e) {}
        ...
        const exited = await new Promise((resolve) => {
          let finished = false;
          const onExit = () => { if (!finished) { finished = true; resolve(true); } };
          this.serverProcess.on('exit', onExit);
          setTimeout(() => { if (!finished) { finished = true; resolve(false); } }, 5000);
        });
